...
let применяется для выражения привязки одного конкретного имени. Простейшая форма let выражения привязывает имя к значению. В качестве значения, можно использовать как числа/числовые выражения, так и строки.
...
Привязанные имена можно использовать после определения, но не в любом месте после let отображения привязки.
